...
from ..utils.tf_utils import multi_label_hot
        ...
        with self.graph.as_default():
            with tf.variable_scope("loss"):
                if self.hp.multilabel:
                    self.per_class_loss = tf.reduce_mean(
                    ...
                    tf.summary.histogram(
                        "per_class_loss", self.per_class_loss, collections=["training"]
                    )
        ...
        with self.graph.as_default():
            with tf.variable_scope("model"):
                ...
                self.one_hot_prediction = (
                    tf.one_hot(self.prediction, self.hp.num_classes)
                    if not self.hp.multilabel
                    else multi_label_hot(self.prediction)
                )
